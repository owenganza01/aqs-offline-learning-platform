// src/components/instructor/LessonManager.tsx
import React, { useState } from 'react';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { storage } from '../../lib/firebase.ts';
import { Course, Lesson } from '../../types.ts';
import { authHeaders, jsonHeaders } from '../../lib/utils.ts';
...
export const LessonManager: React.FC<LessonManagerProps> = ({
  token, selectedCourse, onRefreshCourses, loadCourseFullDetails
}) => {
    ...
    <div className="bg-white border border-slate-150 p-6 rounded-[2rem] shadow-sm relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 pb-2 border-b border-slate-100">
        ...
        <button
          onClick={() => {
            setEditingLessonId(null);
            setLessonForm({ title: '', content: '', videoUrl: '', slidesUrl: '', sortOrder: (selectedCourse.lessons?.length || 0) });
            setShowAddLesson(true);
          }}
          ...
          >
            ...
            <form onSubmit={handleSaveLesson} className="space-y-4">
              <div>
                ...
                <input type="text" required value={lessonForm.title} onChange={e => setLessonForm(l => ({ ...l, title: e.target.value }))} placeholder="e.g. Quantitative Assessment Metrics" className="w-full p-2.5 border border-slate-200 rounded-xl text-xs bg-white outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 transition-all font-semibold text-slate-855" />
              </div>
              <div>
                ...
                <input type="text" value={lessonForm.videoUrl} onChange={e => setLessonForm(l => ({ ...l, videoUrl: e.target.value }))} placeholder="e.g. https://www.youtube.com/embed/dQw4w9WgXcQ" className="w-full p-2.5 border border-slate-200 rounded-xl text-xs bg-white outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 transition-all font-semibold text-slate-855" />
              </div>
              <div>
                ...
                <div className="flex flex-col md:flex-row gap-2">
                  <input type="text" value={lessonForm.slidesUrl} onChange={e => setLessonForm(l => ({ ...l, slidesUrl: e.target.value }))} placeholder="e.g. Google Slides link, OneDrive PowerPoint embed URL, PDF link" className="flex-grow p-2.5 border border-slate-200 rounded-xl text-xs bg-white outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 transition-all font-semibold text-slate-855" />
                  <div className="relative shrink-0">
                    <input type="file" id="slides-file-upload" onChange={handleUploadSlides} accept=".pdf,.ppt,.pptx,.key,.odp" className="hidden" disabled={uploadingSlides} />
                    <label htmlFor="slides-file-upload" className={`h-10 px-4 border border-slate-200 bg-white hover:bg-slate-50 rounded-xl text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 select-none ${uploadingSlides ? 'opacity-50 pointer-events-none' : ''}`}>
                      {uploadingSlides ? (<><RefreshCw className="w-4 h-4 animate-spin text-slate-500" /><span>Uploading...</span></>) : (<><Download className="w-4 h-4 text-slate-500 rotate-180" /><span>Upload PPT/PDF</span></>)}
                    </label>
                  </div>
                </div>
              </div>
              <div>
                ...
                <textarea required value={lessonForm.content} onChange={e => setLessonForm(l => ({ ...l, content: e.target.value }))} placeholder="Provide student reference textbooks or technical content summaries..." className="w-full p-2.5 border border-slate-200 rounded-xl text-xs bg-white h-44 outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 transition-all text-slate-755 font-medium leading-relaxed" />
              </div>
              <div className="flex gap-2">
                <button type="submit" className="bg-pink-600 hover:bg-pink-500 text-white font-bold text-xs px-4 h-10 rounded-xl transition-all shadow-sm active:scale-95 border border-pink-700/20 cursor-pointer">SAVE MATERIALS</button>
                <button type="button" onClick={() => { setShowAddLesson(false); setEditingLessonId(null); }} className="bg-white hover:bg-slate-100 text-slate-600 font-bold text-xs px-4 h-10 rounded-xl border border-slate-200 transition-all shadow-sm cursor-pointer">ABANDON</button>
              </div>
            </form>
          </motion.div>
        )}
      ...
      {(selectedCourse.lessons || []).length === 0 ? (
        ...
        <div className="space-y-2.5">
          {(selectedCourse.lessons || []).map((lesson, index) => (
            <div key={lesson.id} className="bg-slate-50/60 border border-slate-150 p-4 rounded-2xl flex items-center justify-between gap-4 font-sans shadow-sm">
              ...
              <div className="flex items-center gap-1 shrink-0 font-mono">
                <button onClick={() => handleMoveLesson(index, 'up')} disabled={index === 0} title="Move up" className="p-1.5 bg-white border border-slate-200 hover:bg-slate-100 rounded-lg disabled:opacity-30 active:scale-95 cursor-pointer shadow-sm"><ArrowUp className="w-3.5 h-3.5 text-slate-600" /></button>
                <button onClick={() => handleMoveLesson(index, 'down')} disabled={index === (selectedCourse.lessons || []).length - 1} title="Move down" className="p-1.5 bg-white border border-slate-200 hover:bg-slate-100 rounded-lg disabled:opacity-30 active:scale-95 cursor-pointer shadow-sm"><ArrowDown className="w-3.5 h-3.5 text-slate-600" /></button>
                <button onClick={() => handleEditLessonSetup(lesson)} title="Edit" className="p-1.5 bg-indigo-50 border border-indigo-150 hover:bg-indigo-100 rounded-lg ml-1 active:scale-95 cursor-pointer"><Edit3 className="w-3.5 h-3.5 text-indigo-650" /></button>
                <button onClick={() => handleDeleteLesson(lesson.id)} title="Delete" className="p-1.5 bg-red-50 border border-red-150 hover:bg-red-100 rounded-lg active:scale-95 cursor-pointer"><Trash2 className="w-3.5 h-3.5 text-red-600" /></button>
              </div>
            </div>
          ))}
        </div>
